...
class _GroupsPageState extends State<GroupsPage> {
          ...
          IconButton(
              onPressed: () {
                Navigator.push(
                    context,
                    MaterialPageRoute(
                        builder: (context) => const SearchPage()));//search page is linked to this
              },
              icon: const Icon(
  ...
  popUpDialog(BuildContext context) {//popup dialog to create a group, a text field is present to enter group name
    showDialog(
                ...
                children: [
                  _isLoading == true
                      ...
                      : TextField(
                          onChanged: (val) {
                            setState(() {
                              groupName = val;//group name is stored in groupName
                            });
                          },
                  ...
                  onPressed: () async {
                    if (groupName != "") {
                      ...
                      DatabaseService(
                          ...
                          .whenComplete(() {//group is created using createGroup function in database_services.dart
                        _isLoading = false;
                      });
                      Navigator.of(context).pop();
                      showSnackbar(
  ...
  groupList() {//builds list of groups in which user is present and displays them
    return StreamBuilder(
      stream: groups,
      builder: (context, AsyncSnapshot snapshot) {//snapshot of users collection,contains all the data regarding user
        // make some checks
        if (snapshot.hasData) {
            ...
            if (snapshot.data['groups'].length != 0) {
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return GroupTile(//each group is displayed as a tile
                      groupId: getId(snapshot.data['groups'][reverseIndex]),
                      groupName: getName(snapshot.data['groups'][reverseIndex]),
                      userName: snapshot.data['name']);//gets user name from snapshot
                },
              );
            } else {
  ...
  noGroupWidget() {//noGroupWidget is displayed when no groups are present
    return Container(
